  ...
  return (
    <div className="min-h-screen bg-app-dark px-4 py-10 pb-16">
      ...
      {data.programs.map(({id, name}, index) => {
        ...
        return <ActionCard key={id} type="program" color="gradient-3">{name}</ActionCard>
        } else if ((((index + 1) / 3) % 1).toFixed(1) === '0.7') {
          return <ActionCard key={id} type="program" color="gradient-2">{name}</ActionCard>
        } else {
          return <ActionCard key={id} type="program" color="gradient-1">{name}</ActionCard>
        }
      }
      )}
